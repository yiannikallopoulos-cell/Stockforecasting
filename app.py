...
Flask backend with bulletproof error handling and detailed logging.
"""
...
# ── Logging — shows in Render's log viewer ────────────────────────────────────
logging.basicConfig(
    level=logging.INFO,
...
# ── Import model — log any failure clearly ────────────────────────────────────
try:
    from model import (
        fetch_financials, build_assumptions, build_workbook, last_val, cagr
    )
    log.info("model.py imported successfully")
except Exception as e:
    log.error(f"FATAL: Could not import model.py: {e}")
    traceback.print_exc()
...
def compute_dcf(data, assumptions, n_proj=5):
    ...
    dcf_price    = max(0, min(equity_value / scale / shares_mm, price * 20 if price > 0 else 1e6))
    upside       = (dcf_price / price - 1) if price > 0 else 0

    if   upside >= 0.30:  rating,color,desc,stars = "STRONG BUY",  "#1a6b2e","Model implies ≥30% upside — significantly undervalued vs intrinsic value",5
...
def compute_summary(data, assumptions):
    ...
    rev_cagr = safe_float(cagr(rev, 3), 0) if rev else 0
    last_rev  = safe_float(last_val(rev), 0)
    last_ocf  = safe_float(last_val(data.get("op_cf", {})), 0)
    last_capex= abs(safe_float(last_val(data.get("capex", {})), 0))
    return {
        ...
        "proj_rev_growth":[round(safe_float(g)*100,1) for g in assumptions.get("rev_growth",[])],
    }
...
def analyze():
    ...
    log.info(f"[{ticker}] Analysis started — {n_proj} yr projection")

    try:
        log.info(f"[{ticker}] Step 1: fetch_financials")
        data = fetch_financials(ticker)
        log.info(f"[{ticker}] Revenue years: {sorted(data.get('revenue',{}).keys())}")

        log.info(f"[{ticker}] Step 2: build_assumptions")
        assumptions = build_assumptions(data, n_proj)

        log.info(f"[{ticker}] Step 3: compute_dcf")
        ...
        log.info(f"[{ticker}] Step 4: build_workbook")
        ...
        log.info(f"[{ticker}] Complete. Returning response.")
        ...
        if "yfinance" in msg.lower() or "No module" in msg:
            msg = "yfinance not installed — check requirements.txt and redeploy."
            ...
            msg = "Could not reach Yahoo Finance. Please try again in a few seconds."
        elif "NoneType" in msg or "empty" in msg.lower():
            msg = f"Insufficient financial data for '{ticker}'. Try a major US-listed stock."
...
def download(filename):
    ...
    if not path.exists():
        return jsonify({"error": "File not found. Re-run the analysis to regenerate."}), 404
